...
# ✅ Handle surjected_bams sample specification
if "surjected_bams" in config:
    # Get all sample names (keys of surjected_bams)
    config["sample"] = list(config["surjected_bams"].keys())
else:
    config["surjected_bams"] = {}
...
docker_imgs['vg'] = "docker://quay.io/vgteam/vg:v1.59.0"
docker_imgs['vgwork'] = 'docker://quay.io/jmonlong/vg-work:1.59.0_v1'
docker_imgs['gatk_bedtools'] = "docker://docker.io/chiarafolland/custom-gatk-bedtools-glibc"
